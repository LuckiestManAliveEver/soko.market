...
| Offline order intents | Oversell/partial-fulfillment reconciliation, idempotent replay, catalogue/order-intent BLE envelopes, SMS text parsing (clean, ambiguous, unmatched) | See docs/offline/offline-commerce.md; not yet exercised on physical BLE hardware |

Run with the repository-required Node 22.19.x: `pnpm test -- tests/offline-runtime.test.ts tests/offline-runtime-web.test.ts tests/offline-runtime-peer.test.ts tests/offline-runtime-sqlite.test.ts tests/offline-runtime-order-intents.test.ts tests/offline-order-planning.test.ts tests/offline-order-reconciliation.test.ts`. SQLite tests use Node's built-in SQLite solely as the test driver; production can inject better-sqlite3. Also run existing cp7/cp21 sync tests, package/API/web typechecks, the Vite build, formatting/lint and architectural boundary checks. See verification.md for actual results; this matrix does not claim physical-device tests have run.
