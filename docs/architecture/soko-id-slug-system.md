...
**What was *not* converged onto it, and why**: `requirePublicStorefrontBusiness`
...
web service or at `soko-market-api` (a real Node service that *could* do Host-header routing and
...
1. **Telegram `/start` deep-link resolution is not wired up.** `TelegramChannelAdapter.normalizeInbound`
   ...
   captured value (`linkToken`) is consumed by a *different*, security-sensitive, pre-existing
   ...
   linking an *already-known* customer's Telegram account to an existing conversation - not for a
   ...
   real design decision, not a mechanical wiring change - and building a *new* "anonymous stranger
   ...
   `sendMessage` rather than creating a channel link. `getStoreLinks().telegram` (the *generation*
   side) is done and tested; this is only the *receiving* side.
