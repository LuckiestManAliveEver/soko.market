...
`RuntimeModelTemplateRecipe` (`packages/shared-types`) is a _different_ "recipe" concept scoped to
...
checks the entity a mutation _references_ exists before a confirmation token is even minted. Applying
