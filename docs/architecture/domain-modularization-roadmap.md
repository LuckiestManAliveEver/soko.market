...
| 2 | **Logistics & receipts** | `logistics`, `logisticsByInvoice`, `purchaseReceipts`, `receiptLineItems`, `receiptOCRJobs` | Low-medium (unverified against method bodies) | Needs `invoices`, `suppliers` lookups; receipt OCR has its own provider abstraction already (`receipt-ocr-provider.ts`) that's a good template for further sub-splitting. |
