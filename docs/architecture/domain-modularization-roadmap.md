...
entity `Map`s (100 remaining after the commerce, compliance, logistics,
suppliers/sales-agents/receipts, document-imports, and notifications
extractions below) behind one shared auth gate, one generic in-memory
...
foundational rewrite touching all 100 remaining entity types, with a
...
| 6 | **Network/contacts graph** | `networkNodes`, `networkEdges`, `networkSources`, `networkPermissions`, `networkRoutes`, `contactHashes`, `contactHashIdByValue`, `externalIdentities`, `externalIdentityIdBySubject`, `sokoIdentityLinks` | High | The commerce and suppliers domains already depend on this one (injected `networkNodes`/`networkSources` Map references) - extract it *after* its consumers so their deps interfaces are stable, not before. (Notifications turned out not to touch this domain at all - corrected from an earlier, wrong guess in this table.) |
...
count before and after (currently 657 passed / 19 skipped / 1 pre-existing
