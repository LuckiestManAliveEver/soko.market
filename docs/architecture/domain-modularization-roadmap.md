...
entity `Map`s (110 remaining after the commerce, compliance, and logistics
extractions below) behind one shared auth gate, one generic in-memory
snapshot/restore cycle, one generic
...
| 2b | **Purchase receipts & receipt OCR** (split out of the original "Logistics & receipts" row - see correction below) | `purchaseReceipts`, `receiptLineItems`, `receiptOCRJobs` | High | Deliberately deferred to be extracted together with domain 3 (suppliers & sales agents), not on its own - see below. |
| 3 | **Suppliers & sales agents (+ purchase receipts/OCR, 2b above)** | `suppliers`, `salesAgents`, `supplierContactLinks`, `purchaseReceipts`, `receiptLineItems`, `receiptOCRJobs` | High | Needs `networkNodes`/`networkSources` (phonebook contact-matching, same pattern the commerce slice already solved) plus each other bidirectionally: `createReceiptOCRJob`/`confirmReceiptOCRJob` call `matchSupplier`/`requireSupplier`/`createSupplier`/`matchSalesAgent`/`requireSalesAgent`/`createSalesAgent`, while `supplierBusinessCard`/`salesAgentCard` (supplier-domain report builders) call back into `purchaseReceiptsForSupplier`/`purchaseReceiptsForSalesAgent`. Extract as one combined domain rather than two - splitting them would just recreate the same cross-module circular-dependency problem `cp2-error.ts`/`text-normalization.ts` were pulled out to avoid. |
