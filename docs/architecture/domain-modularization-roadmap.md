...
entity `Map`s (90 remaining after the commerce, compliance, logistics,
suppliers/sales-agents/receipts, document-imports, notifications, and
network/contacts-graph extractions below) behind one shared auth gate, one
generic in-memory snapshot/restore cycle,
one generic
...
foundational rewrite touching all 90 remaining entity types, with a
...
count before and after (currently 657 passed / 20 skipped / 1 pre-existing
