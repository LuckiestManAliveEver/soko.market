...
from 6,521 lines to 2,220 lines - a 66% reduction. All 20 state domains
...
`renderActiveWorkspace()`'s switch.
...
architectural parent (`renderActiveWorkspace()`'s switch result renders
*inside* `ChatSurface`'s children) - so it had 101 flat props, mostly
...
1. **Core-context values never relocate.** `session`, `business`,
   ...
   the domain's *actions* that mutate them get extracted, taking the
   relevant setter as a dep.
2. **Getter pattern** (Phase 9, extended in Phase 19/20 to setters): when
   a hook needs something from a hook called *later* in the call
...
7. **Trust the original exploration report over re-derivation.** The one
   ...
   *name* instead of checking the careful clustering done once, up front,
...
`SokoApplication.tsx` at 2,220 lines is still larger than the original
plan's ~800-1,200 estimate - the difference is almost entirely
`renderActiveWorkspace()`'s JSX switch and the main return's JSX tree,
neither of which is state to decompose (the prior roadmap already pulled
every extractable presentational component out). Nothing here calls for a
further hooks/context pass. If `SokoApplication.tsx` needs to shrink
again, the next candidate is the JSX itself - a different kind of
decomposition than either roadmap in this repository has done, and it
would need its own scoped plan.
