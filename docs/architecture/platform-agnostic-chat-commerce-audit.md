...
| OCR retry/failure handling        | PARTIAL     | Worker HTTP retries, timeout, fallback engine, failure codes, manual correction, and a persisted cancel transition exist. No persisted retry transition API exists.                                             |
