...
repository are each individually documented, but the boundary *between* them — which one can
...
*structurally* incapable of proposing a tool at all, not merely policy-blocked from executing one.
...
|pay|send|invite|sync|order|receipt`, case-insensitive). It is a *routing convenience*, not the
...
1. **`cp2_agent_model_bindings`** (`AgentModelBindingSummary`) — the canonical binding for
   *server-side* execution targets (backend/Ollama, OpenAI, installed-app native bridge,
...
2. **`agentModelAssignments`** (`PUT /businesses/:id/agent-model`, `assignAgentModel`) — tracks
   whether a *device-local* model (browser-downloaded or installed-app) has actually completed a
...
## What this document is *not* reporting as a gap
