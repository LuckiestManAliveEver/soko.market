...
- `AgentRuntimeAdapter` ("harness") — the in-process execution interface (`canRun`/`execute`),
  selected via a *third*, independent registry (`AgentRuntimeAdapterDescriptor`,
...
constraint stopping nonsensical combinations. Separately, the harness *registry* let GitHub/Hugging
...
property of *anything* a shop directly chose.
...
3. **The `AgentRuntimeAdapter` execution mechanism itself is unchanged.** The interface
   ...
   all still exist exactly as before. What changed is *who decides the value written there*:
...
- **Fold harness into agent *import*, keep the third-party-code security cap**: rejected in favor
...
so a future GitHub/Hugging-Face-imported *agent* (config only, never executable code) can declare
...
`infra/db/migrations/089_drop_harness_runtime_kind.sql` deletes existing `kind='harness'`
...
`infra/db/migrations/094_agent_definition_runtime_adapter.sql` backfills the existing
