...
export function ChatSurface({
  ...
  // Shared by the desktop sidebar aside and the compact-viewport StackedModule drawer below -
  // `showHeading` is false in the module since StackedModule already renders the "Chats"/"Messages"
  // title in its own heading bar (the notifications toggle still needs to render either way).
  function renderConversationListPanel(showHeading: boolean) {
    return (
      <>
    ...
    <div className={`chat-surface ${showMessageThread && isInboxOpen ? "inbox-open" : ""}`}>
      {showMessageThread ? (
        isCompactViewport ? (
          ...
          >
            {renderConversationListPanel(false)}
          </StackedModule>
        ) : (
          <aside
