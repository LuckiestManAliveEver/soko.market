...
const protocolMigrationSql = readFileSync(
  "infra/db/migrations/083_runtime_handoff_protocol.sql",
  "utf8"
);
const createdAtIndexMigrationSql = readFileSync(
  "infra/db/migrations/084_runtime_handoff_created_at_index.sql",
  "utf8"
);

describe("083 runtime handoff protocol migration", () => {
  it(
    "never reintroduces an index on a non-existent created_at column (regression for Postgres " +
      'error 42703: column "created_at" does not exist - the table definition only ever ' +
      "generated task_id/updated_at, never a plain created_at column, so the original index " +
      "definition failed at deploy time and was removed in commit 5cfe0ed rather than fixed " +
      "in place, since 083 is already applied and this repository never edits an applied " +
      "migration's SQL. The real fix lives in 084, tested below.)",
    () => {
      expect(protocolMigrationSql).not.toMatch(/\btask_id,\s*created_at desc/);
    }
  );
});

describe("084 runtime handoff created-at index migration", () => {
  it("indexes the createdAt JSON field directly rather than a non-existent created_at column", () => {
    expect(createdAtIndexMigrationSql).toContain(
      "on cp2_runtime_handoffs (task_id, (record ->> 'createdAt') desc);"
    );
    expect(createdAtIndexMigrationSql).not.toMatch(/\btask_id,\s*created_at desc/);
  });
  ...
  it(
    ...
    () => {
      expect(createdAtIndexMigrationSql).not.toMatch(
        /generated always as[\s\S]{0,80}::timestamp with time zone/
      );
    }
  );

  it("requires createdAt to be present on every handoff record", () => {
    expect(createdAtIndexMigrationSql).toContain("record ->> 'createdAt' is not null");
  });
  ...
  describePostgres("against a live Postgres instance", () => {
    it(
      "applies cleanly on top of 083 and builds the task/created_at index (regression for the " +
