...
export class PeerProvider implements SokoProvider {
  ...
  private async enqueue(id: string, envelope: PeerOutboxEnvelope): Promise<void> {
    ...
    if (bytes.length > MAX_ENVELOPE_BYTES)
      throw new Error("Peer messages must fit within 64 KiB. Attachments need a separate transport.");
  ...
  private async attemptDelivery(id: string): Promise<void> {
    ...
    if (attempts >= PEER_OUTBOX_RESEND_CAP) {
      ...
      if (entry.envelope.kind === "conversation_message") {
        ...
        for (const handler of this.orderIntentDeliveryFailedHandlers) handler(entry.envelope.intent);
      }
      return;
    }
    ...
    const bytes = new TextEncoder().encode(
      JSON.stringify(
        entry.envelope.kind === "conversation_message" ? entry.envelope.message : entry.envelope.intent
      )
    );
    await this.transmitBytes(
      entry.envelope.kind === "conversation_message" ? KIND_CONVERSATION_MESSAGE : KIND_ORDER_INTENT,
      bytes
    );
  }
  close(): void {
  ...
  private receive(frame: Uint8Array): void {
    if (
      ...
      return;
    const kind = frame[2];
    if (kind !== KIND_CONVERSATION_MESSAGE && kind !== KIND_CATALOGUE_DIGEST && kind !== KIND_ORDER_INTENT)
      return;
    const now = Date.now();
