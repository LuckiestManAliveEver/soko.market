...
// across instances, instead of living only in this process's memory. The rate-limit plugin
// is registered with `skipOnError: true`, so if this connection is ever down, requests are
// still served (rate limiting is skipped, not fatal to the inference service).
