...
**Format correction (added after implementation):** the concept below proposes a
`countryA########` numeric format. The shipped implementation does not use this format.
`Cp2Store.createGlobalShopId` (`services/api/src/cp2/store.ts`) generates IDs as `soko.<handle>`
slugs derived from the business name (e.g. `soko.mama-mboga`, with `soko.mama-mboga-2` on
collision). `tests/sokoid-and-storefront.test.ts` asserts the original `254A########` shape is
invalid. The country-namespace/business-agent-prefix concept was dropped during implementation.
`docs/architecture/soko-id-slug-system.md` is the current authoritative spec for the actual format,
collision handling, rename/cooldown rules, and the shared resolver — read it instead of relying on
the concept text below for anything format-specific.
...
- use a compact, punctuation-free identifier pattern (implemented as `soko.<handle>`, not the
  originally proposed `countryAidentifier` pattern — see format correction above)
- support copy/share affordances for packaging, receipts, and QR-code workflows

## Implemented Surfaces

- Every new business receives a stable `sokoId` in the `soko.<handle>` slug format (for example,
  `soko.mama-mboga`), generated by `Cp2Store.createGlobalShopId`, not the `countryA########` format
  originally proposed in the source concept.
- Numeric-suffix collision handling produces `soko.mama-mboga-2`, `soko.mama-mboga-3`, etc.
...
- Lookup normalization is limited to trim/lowercase (`normalizeStorefrontLookupId`,
  `services/api/src/cp2/text-normalization.ts`); there is no code that converts a legacy
  `+country-A########` or `countryA########` identifier into the `soko.<handle>` format — no
  business was ever assigned an ID in that format, so no such migration was needed.
